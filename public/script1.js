...
// Replace static massagers data with API call
async function fetchMassagers(page = 1, limit = 10) {
  try {
    const token = localStorage.getItem('dimple_token');
    const response = await fetch(`/api/massagers?page=${page}&limit=${limit}`, {
      headers: {
    ...
    if (data.success) {
      ...
      console.error('Failed to fetch massagers:', data.message);
      return { data: [], total: 0, pages: 1 };
    }
  } catch (error) {
    console.error('Error fetching massagers:', error);
    return { data: [], total: 0, pages: 1 };
  }
}

// Update renderMassagers function to use API data
async function renderMassagers() {
  massagersContainer.innerHTML = '';
  
  const massagersData = await fetchMassagers(currentPage, itemsPerPage);
  
  if (massagersData.data.length === 0) {
    massagersContainer.innerHTML = '<p>No massagers available</p>';
    return;
  }
  
  massagersData.data.forEach(massager => {
    const card = document.createElement('div');
    card.className = 'massager-card';
    card.innerHTML = `
  ...
  // Update pagination
  renderPagination(massagersData.total, massagersData.pages);
